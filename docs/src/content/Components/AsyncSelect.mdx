...
import {
  ...
  CustomRenderPreview,
  GenericTypesPreview,
} from '@/components/@Components/AsyncSelect';
  ...
  tabs={[
    {
      ...
      code: `import { AsyncSelect } from "@kousta-ui/components";
...
// Called when the dropdown opens, when it is scrolled to the end, and on every
// search. It is handed the page to load and whatever the user typed.
...
export function Example() {
  return (
    <AsyncSelect<Product>
      ...
      getData={getProducts}
      // pull the rows out of whatever shape your API returns
      extractDynamicData={(response: ProductsResponse) => response.products}
      // say whether another page exists, so it knows when to stop asking
      hasMore={(response: ProductsResponse, page) => page < response.meta.last_page}
...
// Called when the dropdown opens, when it is scrolled to the end, and on every
// search. It is handed the page to load and whatever the user typed.
async function getProducts({ page, limit, searchTerm }) {
  const params = new URLSearchParams({
    page: String(page),
    limit: String(limit),
  });
