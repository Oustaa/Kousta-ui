...
export default function Example() {
  ...
  preview={<StaticPaginationPreview />}
  defaultTab="ts"
/>
...
Don't use `pagination.type = "static"` with `actions.get` (API-driven data). It
can lead to empty pages, because the table slices the already-paginated data a
second time.
</Callout>
...
export default function Example() {
  ...
  preview={<DynamicPaginationPreview />}
  defaultTab="ts"
/>
