...
| `sortBy` | `true` | Marks the column sortable — see [Sorting](./Sorting). Requires `value` (not `exec`). The type also accepts a `{ name?: string }` object, but only `true` has an effect today. |
...
type THeaderSort = {
  name?: string; // declared in the type, not read by the current implementation
};
```
...
| `sort` | [`SortProps`](#sortprops) | Remaps the params sent to `actions.get` when a sortable column is clicked — see [Sorting](./Sorting). Same shape and effect as `config.sort`; this one takes precedence if both are set. |
...
| `sort` | [`SortProps`](#sortprops) | Same as `options.sort` — see [Sorting](./Sorting). `options.sort` takes precedence if both are set. |
